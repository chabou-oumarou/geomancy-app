...
UI_TEXT = {
    "EN": {
        "title": "The Golden Star Shield", "subtitle": "16 Figures of Ancient Wisdom", "btn": "Generate Full Shield",
        "mother": "Mother", "daughter": "Daughter", "nephew": "Nephew", "row": "Row",
        "foundation": "I. Mothers & Daughters", "nephews_sec": "II. The Nephews", "court": "III. The Verdict",
    ...
    "FR": {
        "title": "Le Blason des Étoiles d'Or", "subtitle": "Les 16 Figures de la Sagesse Ancienne", "btn": "Générer le Blason Complet",
        "mother": "Mère", "daughter": "Fille", "nephew": "Neveu", "row": "Ligne",
        "foundation": "I. Mères & Filles", "nephews_sec": "II. Les Neveux", "court": "III. Le Verdict",
...
def render_star_mirror(input_str):
    ...
    stars = "★ " * len(clean)
    return f"<div style='color: #D4AF37; font-size: 1.1rem; margin-top: -15px; margin-bottom: 5px;'>{stars}</div>"

def render_card(fig, label, color="#D4AF37", size="30px", glow=False):
    glow_style = f"box-shadow: 0 10px 40px {color}44;" if glow else "box-shadow: 0 4px 15px rgba(0,0,0,0.05);"
...
# --- 3. APP UI ---
st.set_page_config(page_title="Star Shield", layout="wide")
...
st.markdown("""
    ...
    h2 { color: #D4AF37 !important; margin-top: 40px !important; border-bottom: 2px solid #edf0f2; padding-bottom: 10px; }
    .stTextInput input { border-radius: 12px; border: 2px solid #edf0f2; height: 40px; }
    </style>
    """, unsafe_allow_html=True)

# Language / Reset Sidebar
...
if st.button(T["btn"], use_container_width=True, type="primary"):
    ...
    # CALCULATIONS FOR ALL 16 FIGURES
    D = [[M[j][i] for j in range(4)] for i in range(4)] # Daughters (4)
    N = [add_figs(M[0], M[1]), add_figs(M[2], M[3]), add_figs(D[0], D[1]), add_figs(D[2], D[3])] # Nephews (4)
    RW = add_figs(N[0], N[1]) # Witness 1
    LW = add_figs(N[2], N[3]) # Witness 2
    Judge = add_figs(RW, LW) # Judge (1)
    Reconciler = add_figs(Judge, M[0]) # Reconciler (1)
    
    GOLD = "#D4AF37"

    # I. DISPLAY MOTHERS & DAUGHTERS (8)
    st.header(T["foundation"])
    f_cols = st.columns(8)
    for i in range(4):
        f_cols[i].markdown(render_card(M[i], f"M{i+1}", GOLD), unsafe_allow_html=True)
        f_cols[i].caption(f"<center>{GEOMANTIC_DATA[tuple(M[i])]['name']}</center>", unsafe_allow_html=True)
    for i in range(4):
        f_cols[i+4].markdown(render_card(D[i], f"D{i+1}", GOLD), unsafe_allow_html=True)
        f_cols[i+4].caption(f"<center>{GEOMANTIC_DATA[tuple(D[i])]['name']}</center>", unsafe_allow_html=True)

    # II. DISPLAY NEPHEWS (4)
    st.header(T["nephews_sec"])
    n_cols = st.columns(4)
    for i in range(4):
        n_cols[i].markdown(render_card(N[i], f"N{i+1}", GOLD, size="35px"), unsafe_allow_html=True)
        n_cols[i].caption(f"<center>{GEOMANTIC_DATA[tuple(N[i])]['name']}</center>", unsafe_allow_html=True)

    # III. DISPLAY COURT (WITNESSES, JUDGE, RECONCILER) (4)
    st.header(T["court"])
    
    # Witnesses
    w_c1, w_c2 = st.columns(2)
    w_c1.markdown(render_card(RW, T["witness_r"], GOLD, size="40px"), unsafe_allow_html=True)
    w_c2.markdown(render_card(LW, T["witness_l"], GOLD, size="40px"), unsafe_allow_html=True)
    ...
    with res_j:
        ...
        st.markdown(render_card(Judge, T["judge"], GOLD, size="55px", glow=True), unsafe_allow_html=True)
        st.markdown(f"<div style='background: white; padding: 25px; border-radius: 20px; border-bottom: 5px solid {GOLD};'><h3>{j_info['name']}</h3>{j_info['meaning'][L]}</div>", unsafe_allow_html=True)
    
    with res_r:
        rec_info = GEOMANTIC_DATA[tuple(Reconciler)]
        st.markdown(render_card(Reconciler, T["reconciler"], GOLD, size="55px", glow=True), unsafe_allow_html=True)
        st.markdown(f"<div style='background: white; padding: 25px; border-radius: 20px; border-bottom: 5px solid {GOLD};'><h3>{rec_info['name']}</h3><b>{rec_info['element']}</b><br><br>{rec_info['meaning'][L]}</div>", unsafe_allow_html=True)
