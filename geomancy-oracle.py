...
def render_card(fig, label, color=MAROON, size="35px", glow=False):
    ...
    return f"""<div style="background: white; border: 1px solid #edf0f2; border-radius: 20px; padding: 20px; text-align: center; {glow_style}"><div style="font-size: 0.65rem; color: #a0a0a0; font-weight: 800; text-transform: uppercase; margin-bottom: 12px; letter-spacing: 1.5px;">{label}</div>{rows}</div>"""
...
if st.button(T["btn"], use_container_width=True, type="primary"):
    ...
    with res_j:
        ...
        st.markdown(f"<div style='background: white; padding: 25px; border-radius: 20px; border-bottom: 6px solid {MAROON};'><h3>{j_info['name']}</h3>{j_info['meaning'][L]}</div>", unsafe_allow_html=True)
    with res_r:
        ...
        st.markdown(f"<div style='background: white; padding: 25px; border-radius: 20px; border-bottom: 6px solid {MAROON};'><h3>{r_info['name']}</h3><b>{r_info['element']}</b><br><br>{r_info['meaning'][L]}</div>", unsafe_allow_html=True)
